...
if __name__ == '__main__':
    ...
    if 'URI_Doc' in config['DOCUMENT'] and config['DOCUMENT']['URI_Doc'] == 'yes':
        ...
        walks_,instances = get_rdf2vec_walks(graph1 = graph1, graph2 = graph2, mapping_file=mapping, walker_type=config['DOCUMENT']['walker'],
        ...
        with open(os.path.join(config['DOCUMENT']['cache_dir'], 'new_axioms.txt'), 'w', encoding="utf-8") as f:
            for x in walks_:
                ax = "\t".join(x)
                f.write('%s\n' % ax)
